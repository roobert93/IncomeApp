...
export default function App() {
  ...
  const addIncome = () => {
    if (amount > 0) {
      ...
      handleAlert({
        type: "redAlert",
        text: "Your value must be bigger that zero"
      });
    }
    setAmount("");
  };
  ...
  const together = () => {
    if (buyAmount > 0 && text !== "") {
      if (buyAmount > income) {
        ...
        handleAlert({ type: "greenAlert", text: "Your item was added." });
      }
      setTotal([...total, { text, buyAmount, id: randomId() }]);
      setIncome(income - buyAmount);
    } else {
    ...
    <div className="full">
      ...
      {income !== "" && (
        <>
          <h1>
            {" "}
            <div className={color()}>{income}$ </div>for this month
          </h1>
          <input
            type="text"
            value={text}
            onChange={handleText}
            placeholder="what you paying?"
          ></input>
          <input
            type="number"
            value={buyAmount}
          ...
          <ul>
            {total.map(a => {
              return (
                <li key={a.id}>
                  <span className="textLi">{a.text}</span>
                  <span className="amountLi">{a.buyAmount}$</span>
                  <button
                    className="buttonLi"
                    onClick={() => deleteItem(a.id, a.buyAmount)}
                  >
                    -
                  </button>
                </li>
